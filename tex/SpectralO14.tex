...
\begin{document}
  ...
  $\hat{\delta}_{\mathrm{exact}}$, and separating exact-block identities from the additional
  requirements needed to define endpoint and cross-substrate diagnostics.
  ...
  \subsection{Logical status of the central objects}
    ...
    \begin{description}
      ...
      $G_q$, and a fitting window evaluated against conditions~(E1$'$)--(E3)).
      The mean fit can be well defined when the block-homogeneity condition~(E2) fails.
  ...
  \section{Exact Central-Phase Invariance}
